...
// Normalize any stored plan into { Day: [recipeId, ...] }
function normalizePlan(obj) {
  const base = Object.fromEntries(DAYS.map(d => [d, []]));
  if (!obj || typeof obj !== 'object') return base;
  for (const d of DAYS) {
    const v = obj[d];
    if (Array.isArray(v)) base[d] = v.filter(Boolean);
    else if (typeof v === 'string') base[d] = [v];   // migrate old single-value plan
    else base[d] = [];                                // null / missing -> []
  }
  return base;
}

const getPlan = () => normalizePlan(JSON.parse(localStorage.getItem(LS_KEYS.PLAN) || 'null'));
const setPlan = plan => localStorage.setItem(LS_KEYS.PLAN, JSON.stringify(normalizePlan(plan)));
...
// === Render: Weekly Plan (multiple meals per day) =====================
function renderPlan() {
  const plan = getPlan();
  ...
  planTable.innerHTML = DAYS.map(d => {
    ...
    const rows = (ids && ids.length)
      ? ids.map((rid, idx) => {
          ...
          if (!r) return '';
          return `
            <div class="row" style="justify-content:space-between; padding:4px 0; border-top:1px dashed #e5e7eb">
              <div>
                <span class="title">${r.title}</span>
                <span class="pill">${r.minutes} min</span>
              </div>
              <button class="ghost removeOne" data-day="${d}" data-index="${idx}" title="Remove this meal">Remove</button>
            </div>`;
        }).join('')
      : '<span class="muted">Empty</span>';

    // Keep a simple "Clear day" button in the right-most cell
    return `<tr>
      <th scope="row">${d}</th>
      <td>${rows}</td>
      <td style="width:1%"><button data-day="${d}" class="ghost clearDay">Clear</button></td>
    </tr>`;
  }).join('');

  // Clear entire day
  planTable.querySelectorAll('button.clearDay').forEach(btn => {
  ...
  // Remove a single meal from a day
  planTable.querySelectorAll('button.removeOne').forEach(btn => {
    btn.addEventListener('click', e => {
      const d = e.currentTarget.getAttribute('data-day');
...
function attachCardHandlers(id) {
  ...
  addBtn?.addEventListener('click', () => {
    ...
    const plan = getPlan();
    plan[day].push(id);               // <-- append (multiple meals supported)
    setPlan(plan);
    daySel.value = '';
    renderPlan();
  });
...
// Keep tabs in sync (multi-tab support)
